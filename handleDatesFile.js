...
function readDatesFile(fileName) {
  ...
  for (let i=1; i<rows.length; i++) {
    let [ name1, name2, matchscore, email1, email2, slot, startTime, endTime, dailyRoomName, status, ...rest ] = rows[i].split(',')
    dates[i-1] = { name1, name2, matchscore, email1, email2, slot, startTime, endTime, dailyRoomName, status }
  }
  return dates
}
